...
The library provides 9 residue handling methods (fewer in RFC_MINIMAL mode):
...
.. code-block:: c
   ...
   double damage;
   RFC_damage(&ctx, &damage, NULL);  // NULL: ignore residue damage
...
**Description:** Call ``RFC_finalize()`` without processing the residue and without
incorporating the pending interim turning point. The context transitions to
``RFC_STATE_FINISHED`` with only the cycles closed during ``RFC_feed`` counted.

**Behavior:**

- Residue is left unprocessed (similar to ``RFC_RES_IGNORE``)
- Unlike ``RFC_RES_IGNORE``, the interim turning point is **not** incorporated
- State becomes ``RFC_STATE_FINISHED`` — further ``RFC_feed`` calls are not possible

.. note::

   For chunked/streaming processing, do **not** call ``RFC_finalize()`` between
   chunks. Simply call ``RFC_feed()`` repeatedly — the context stays in
   ``RFC_STATE_BUSY`` and accumulates cycles across all chunks. Call
   ``RFC_finalize()`` once with the desired residue method when all data is fed.

**When to Use:**

- Explicitly abandoning a counting run without residue processing
- Obtaining intermediate closed-cycle results where the last partial turning
  point should be excluded

**Example (chunked streaming — no RFC_finalize between chunks):**
...
.. code-block:: c
   ...
   // Feed data in chunks — do NOT call RFC_finalize between chunks
   RFC_feed(&ctx, chunk1, chunk1_len);
   RFC_feed(&ctx, chunk2, chunk2_len);
   RFC_feed(&ctx, chunk3, chunk3_len);

   // Finalize once all data has been fed
...
- May require ``RFC_AR_SUPPORT`` (auto-resize support) enabled
...
.. code-block:: python
   ...
   methods = [
       (rfcnt.ResidualMethod.NONE,       "NONE"),
       (rfcnt.ResidualMethod.HALFCYCLES,  "HALFCYCLES"),
       (rfcnt.ResidualMethod.FULLCYCLES,  "FULLCYCLES"),
       (rfcnt.ResidualMethod.REPEATED,    "REPEATED"),
   ]
   ...
   for method, name in methods:
       ...
       print(f"{name:12s}: damage = {result['damage']:.6e}")
...
Examine residue content before choosing method. ``RFC_res_get()`` is available
at any time after ``RFC_init()`` — including while the context is still in
``RFC_STATE_BUSY`` (i.e. before ``RFC_finalize()`` is called):

.. code-block:: c

   rfc_ctx_s ctx;
   // ... feed data ...
   // Do NOT call RFC_finalize yet — RFC_res_get works while state is BUSY
   ...
   // Decide based on residue characteristics, then finalize once
...
When using damage history (``RFC_DH_SUPPORT``), residue damage increments are
distributed over the damage history according to the spread-damage method.
``dh[i]`` stores the damage **increment** at sample ``i``, not a cumulative value.
Use ``ctx.damage`` for the total, or ``np.cumsum(dh)`` for the cumulative timeline:
...
.. code-block:: c
   ...
   // Sum all increments for total damage (equals ctx.damage)
   double total = 0.0;
   for (size_t i = 0; i < dh_count; i++) total += dh[i];
...
**4. Unexpected behavior with RFC_RES_NO_FINALIZE**
   ...
   **Solution:** Must call RFC_finalize() with proper method before accessing results:
   ...
   .. code-block:: c
      ...
      RFC_damage(&ctx, &damage, NULL);  // OK now
