...
**Description:** Spread damage proportional to amplitude change from P2 to P4 (same weighting as RAMP_AMPLITUDE_23, extended over the full P2→P4 range).
...
.. code-block:: c
   ...
   // dh[i] = damage INCREMENT at sample i (not cumulative)
   // Sum all increments for total, or accumulate for timeline
...
.. code-block:: python
   ...
   dh = result['dh']               # Damage increments at each sample (not cumulative)
   damage_history = np.cumsum(dh)  # Cumulative damage over time
...
.. code-block:: python
   ...
   result = rfcnt.rfc(
       ...
       residual_method=rfcnt.ResidualMethod.REPEATED
   )

   # dh stores damage increments; use cumsum for cumulative timeline
   import matplotlib.pyplot as plt
   plt.plot(np.cumsum(result['dh']))
   plt.xlabel('Sample')
...
The cumulative damage timeline (``np.cumsum(dh)``) is piece-wise linear and
...
.. code-block:: python
   ...
   # Smooth the cumulative curve
   f = interp1d(range(len(cumulative)), cumulative, kind='cubic')
   t_smooth = np.linspace(0, len(cumulative) - 1, len(cumulative) * 10)
   dh_smooth = f(t_smooth)

   plt.plot(t_smooth, dh_smooth, label='Smoothed cumulative')
   plt.plot(cumulative, 'o', markersize=2, label='Original cumulative')
   plt.ylabel('Cumulative Damage')
   plt.legend()
   plt.show()
