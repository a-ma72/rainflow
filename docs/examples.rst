...
.. code-block:: python
   ...
   result_astm = rfcnt.rfc(
       ...
       class_width=5.0,
       use_ASTM=True,
       residual_method=rfcnt.ResidualMethod.FULLCYCLES
   )
   ...
   # Plot damage increments and cumulative damage over time
   import matplotlib.pyplot as plt
   import numpy as np
   plt.plot(np.cumsum(result_dh['dh']))
   plt.xlabel('Sample index')
   plt.ylabel('Cumulative damage')
   plt.title('Damage History (cumulated)')
   plt.show()
...
.. code-block:: python
   ...
   tp = result['tp']
   tp_time = time[tp[:, 0].astype(int) - 1]  # tp[:,0] is 1-based index into data
   axes[0, 0].plot(tp_time, tp[:, 1], 'ro', markersize=3, label='Turning points')
   ...
   # Damage history (cumulated for visualization)
   axes[1, 0].plot(np.cumsum(result['dh']))
   axes[1, 0].set_xlabel('Sample')
   axes[1, 0].set_ylabel('Cumulative Damage')
   axes[1, 0].set_title('Damage History (cumulated)')
   axes[1, 0].grid(True)
