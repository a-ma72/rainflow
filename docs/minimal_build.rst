...
.. code-block:: c
   ...
   #define RFC_AR_SUPPORT       OFF    // No auto-resize support (RFC_FLAGS_AUTORESIZE)
...
+-------------------------+------------------+-----------------------------+
| RFC_AR_SUPPORT          | Disabled         | No auto-resize buffers      |
+-------------------------+------------------+-----------------------------+
...
.. note::
   ...
   ``RFC_wl_init_modified()`` and ``RFC_wl_init_original()`` are not available
   in RFC_MINIMAL. Only the elementary Wöhler curve is supported.

**Results:**

.. note::

   ``RFC_damage()``, ``RFC_rfm_get()``, ``RFC_rp_get()``, and ``RFC_lc_get()``
   are **not available** in RFC_MINIMAL. Access results directly from the context struct:

   .. code-block:: c

      double damage     = ctx.damage;                /* scalar: cumulated damage from closed cycles */
      double dam_res    = ctx.damage_residue;         /* damage attributed to residue after finalize */
      rfc_counts_t *rfm = ctx.rfm;                   /* dense matrix, class_count² elements, row-major */

   ``ctx.rp`` and ``ctx.lc`` are also not available in RFC_MINIMAL.
...
.. code-block:: c
   ...
   int main(void)
   {
       ...
       /* RFC_damage() not available in RFC_MINIMAL — access ctx.damage directly */
       printf("Total damage: %.6e\n", ctx.damage);
       printf("Full cycles: %.1f\n", ctx.full_inc);
       printf("Half cycles: %.1f\n", ctx.half_inc);

       /* RFC_rfm_get() not available in RFC_MINIMAL — access ctx.rfm directly.
          ctx.rfm is a dense class_count×class_count matrix, row-major (row=from, col=to). */
       const rfc_counts_t *rfm = ctx.rfm;
       printf("\nRainflow Matrix (non-zero entries):\n");
       for (size_t i = 0; i < ctx.class_count; i++)
       {
           for (size_t j = 0; j < ctx.class_count; j++)
           {
               rfc_counts_t cnt = rfm[i * ctx.class_count + j];
               if (cnt > 0)
               {
...
.. code-block:: c
   ...
   /* Custom allocator that serves requests from pre-allocated static pools */
   static void *static_mem_alloc(void *ptr, size_t num, size_t size, int aim)
   {
       /* aim == RFC_MEM_AIM_RFM: rfm_buffer, aim == RFC_MEM_AIM_RESIDUE: residue_buffer */
       if (num == 0) return NULL;  /* free */
       if (aim == RFC_MEM_AIM_RFM) return rfm_buffer;
       if (aim == RFC_MEM_AIM_RESIDUE) return residue_buffer;
       return NULL;
   }

   void embedded_rainflow_init(void)
   {
       /* Set custom allocator BEFORE RFC_init so init uses static pools */
       memset(&ctx, 0, sizeof(ctx));
       ctx.mem_alloc = static_mem_alloc;
...
If your system doesn't have malloc(), provide a custom allocator via the
``ctx.mem_alloc`` function pointer before calling ``RFC_init()``:

.. code-block:: c

   void *my_mem_alloc(void *ptr, size_t num, size_t size, int aim)
   {
       if (num == 0) { my_static_free(ptr); return NULL; }
       return my_static_alloc(num * size);
   }

   rfc_ctx_s ctx;
   memset(&ctx, 0, sizeof(ctx));
   ctx.mem_alloc = my_mem_alloc;
   RFC_init(&ctx, 50, 2.0, 0.0, 1.0, RFC_FLAGS_DEFAULT);
...
.. code-block:: c
   ...
   /* RFC_damage() not available in RFC_MINIMAL — use ctx.damage directly */
