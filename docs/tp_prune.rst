...
If you need both damage history and bounded TP memory, choose one of these
approaches:

- Disable damage history (``RFC_SD_NONE``) and use TP pruning for memory control.
- Enable damage history but skip TP storage (don't call ``RFC_tp_init()``).
- Process data in fixed-length blocks without damage history, accumulating
  histograms across blocks.
