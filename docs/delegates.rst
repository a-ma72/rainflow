...
**D. tp_inc_damage_fcn - Increment Turning Point Damage**

**Type:** ``rfc_tp_inc_damage_fcn_t``

**Signature:**

.. code-block:: c

   typedef bool (*rfc_tp_inc_damage_fcn_t)(
       rfc_ctx_s *rfc_ctx,
       size_t tp_pos,       // Position (1-based)
       double damage        // Damage increment to add
   );

**Purpose:** Add a damage increment to a turning point in custom storage.

**When Called:** When the damage spreading logic attributes damage to a specific turning point (requires ``RFC_DH_SUPPORT``).

**Use Cases:**

- Accumulate per-turning-point damage in a database or file
- Forward damage increments to an external fatigue model

**Example:**

.. code-block:: c

   bool my_tp_inc_damage(rfc_ctx_s *ctx, size_t tp_pos, double damage)
   {
       if (!tp_pos || tp_pos > external_tp_capacity)
           return false;

       external_tp_storage[tp_pos - 1].damage += damage;
       return true;
   }

   ctx.tp_inc_damage_fcn = my_tp_inc_damage;
...
.. warning::
   ...
   When using the C++ wrapper (``rainflow.hpp``), ``internal.obj`` is set by
   ``rf.init()`` to the ``RainflowT`` instance pointer and must not be
   overwritten. The built-in ``tp_set_fcn``, ``tp_get_fcn``, and
   ``tp_inc_damage_fcn`` delegates all cast ``internal.obj`` back to
   ``RainflowT*``, and ``deinit()`` uses it to verify ownership. Overwriting
   this field silently breaks turning point storage and destruction.

   Use ``internal.obj`` only when working directly with the **C API**
   (``rainflow.h``). In C++, pass custom context via a lambda capture or
   a struct referenced from within the delegate's own static wrapper.
...
.. code-block:: cpp
   ...
   public:
       ...
       CustomRainflow()
       {
           ...
           // Do NOT touch rf.ctx_get().internal.obj — the C++ wrapper already
           // stores its own 'this' pointer there for tp_set/tp_get/tp_inc_damage.
           // Pass custom context via a pointer stored in the outer class instead.

           // Set delegate (static wrapper recovers 'this' from the outer object)
           rf.ctx_get().damage_calc_fcn = &CustomRainflow::damage_calc_static;
       }

       // Static wrapper — receives the rfc_ctx_s* but must NOT use internal.obj.
       // Recover the outer object pointer through a side-channel (e.g. global/TLS)
       // or redesign so the lambda/functor holds the reference directly.
       static bool damage_calc_static(rfc_ctx_s *ctx, unsigned from_class,
                                     unsigned to_class, double *damage, double *Sa)
       {
           // Example: outer pointer stored separately, not via internal.obj
           CustomRainflow *self = s_instance;
   ...
   private:
       ...
       static CustomRainflow *s_instance;  // Side-channel — or use thread-local storage
   };
...
1. **Memory Allocation is Not a Delegate**

   Custom allocation is supported via the ``mem_alloc`` function pointer
   (``rfc_mem_alloc_fcn_t``) set in ``rfc_ctx_s``, but it is independent of
   the ``RFC_USE_DELEGATES`` compile flag. Set ``ctx.mem_alloc`` before calling
   ``RFC_init()`` to use a custom allocator.
